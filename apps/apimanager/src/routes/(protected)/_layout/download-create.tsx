...
export const Route = createFileRoute("/(protected)/_layout/download-create")({
	...
	switch (provider) {
		case "winget":
			return packageId
				...
				: "";
		case "chocolatey":
			return packageId ? `choco install ${packageId} -y` : "";
		case "custom":
			return "";
	}
}
...
function DownloadCreatePage() {
	...
	useEffect(() => {
		if (provider !== "custom" && packageId) {
			const newCommand = generateCommand(provider, packageId);
			setSingleCommand(newCommand);
			if (installType === "multi") {
				const updatedCommands = [...commands];
				if (updatedCommands[0] === "" || isAutoGeneratedCommand(updatedCommands[0])) {
					updatedCommands[0] = newCommand;
	...
	const isAutoGeneratedCommand = (cmd: string): boolean => {
		const wingetPattern = /^winget install --id .* --accept-source-agreements --accept-package-agreements$/;
		const chocoPattern = /^choco install .* -y$/;
		return wingetPattern.test(cmd) || chocoPattern.test(cmd);
	};
	...
	const handleProviderChange = (newProvider: DownloadProvider) => {
		...
		if (newProvider === "custom") {
			...
			const newCommand = generateCommand(newProvider, packageId);
			setSingleCommand(newCommand);
		}
		setCommands(["", ""]);
	};
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (apiError.details) {
				...
				toast.error(firstError ?? "Failed to create download");
			} else {
				toast.error(apiError.error ?? "Failed to create download");
			}
		} finally {
		...
		<>
			<div className="mb-8">
				...
				<h1 className="font-bold text-3xl text-zinc-900 dark:text-zinc-100">
					Compose a download blueprint
				</h1>
				<p className="mt-1 text-zinc-600 dark:text-zinc-400">
					Pick your provider, define commands or scripts, and capture every asset
					the desktop app needs to ship a polished install experience.
				</p>
			</div>
						...
						{provider === "custom" ? (
							...
							<div className="grid gap-6 sm:grid-cols-2">
								...
								<div className="space-y-2">
									...
									/>
									<p className="text-xs text-zinc-500 dark:text-zinc-400">
										Auto-generates the download command when entered.
									</p>
								</div>
							</div>
						)}
